...
export default function AdminCompanyPage() {
  ...
  async function deleteTenantPermanently() {
    ...
    const proceed = window.confirm(
      `Permanently delete tenant "${company.company_name}" and all workspace data (logins, FIR customers, parts, invoices, FIR intelligence rows)? This cannot be undone.`,
    );
    if (!proceed) return;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border border-red-900/50 bg-red-950/15 p-6 space-y-3">
        ...
        <p className="text-sm text-slate-400">
          Removes this company and all workspace data (users, FIR customers, parts, invoices, logged FIR rows). Deleting
          users on the Users page only removes logins — the company would still appear here until you delete the tenant.
        </p>
